/* ============================================================================
   File: tracker.js
   Deployment Timestamp: Sat, Aug 22, 2026, 22:37 (EDT - New York)
   ...
   Notes: Works seamlessly on HTTP & HTTPS. Strict in-game collectible sequence
          (PL -> CD -> HI -> SE -> WB) across all 7 missions with unique YT clips.
   ============================================================================ */

/* === SECTION: Dynamic Layout Styles (Prevents CSS Card Clipping) === */
// Line 14: Injects essential grid rules so no items get cut off on mobile/desktop
...
// Line 41: Universal Protocol-relative dynamic module imports
...
// Line 46: Firebase Entertainment Configuration
...
// Line 59: Gamertag Theme Color Palette
const userThemes = {
...
// Line 68: Strict In-Game Item Type Order
...
/* === SECTION: Master Collectibles Dataset (133 Total Verified Items) === */
// Line 78: Complete item database sorted by mission and authentic category order
const sniperData = [
    // =========================================================================
    ...
    // =========================================================================
    // MISSION 10: WOLF MOUNTAIN (19 Items)
    // =========================================================================
    // Personal Letters (5)
...
const appState = {
    ...
    getDocRef: function() {
        // Line 283: Direct Firestore document reference for player progress
    ...
    loadHunter: function(gamertag) {
        ...
        this.unsub = onSnapshot(docRef, (snap) => {
            ...
            console.warn("Firestore snapshot error (using fallback):", error.message);
            this.loadHunterFromLocalStorage(gamertag);
        });
    },
    ...
    render: function() {
        ...
        cats.forEach(cat => {
            ...
            // Strict In-Game Sequence: PL -> CD -> HI -> SE -> WB
...
// Line 471: Loads navigation options dynamically from Google Sheets
...
// Line 549: Dropdown menu click handler
